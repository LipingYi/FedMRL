...
from experiments.FLHetero.Models.CNNs import CNN_1_large,CNN_1_small,projector
...
def train(data_name: str, data_path: str, classes_per_node: int, num_nodes: int, fraction: float,
    ...
    if data_name == "cifar10":
        net_large = CNN_1_large(n_kernels=n_kernels)
        net_small = CNN_1_small(n_kernels=n_kernels, out_dim=10, small_rep_dim=small_rep_dim)
        net_proj = projector(in_dim=small_rep_dim+500, out_dim=500)
    elif data_name == "cifar100":
        net_large = CNN_1_large(n_kernels=n_kernels, out_dim=100)
        net_small = CNN_1_small(n_kernels=n_kernels, out_dim=100, small_rep_dim=small_rep_dim)
        net_proj = projector(in_dim=small_rep_dim+500, out_dim=500)
    ...
    orininal_file = str(save_path / f"Homo_FedMRL2_small_rep_dim_{small_rep_dim}_N{num_nodes}_C{fraction}_R{steps}_E{epochs}_B{bs}_NonIID_{data_name}.csv")
    with open(orininal_file, 'w', newline='') as file:
        ...
        for step in step_iter:  # step is round
            ...
            for c in select_nodes:
                ...
                for i in range(epochs):
                    ...
                    for j, batch in enumerate(nodes.train_loaders[node_id]):
                        ...
                        net_small.fc3.weight.data = net_large.fc3.weight.data[:, :small_rep_dim]
                        net_small.fc3.bias.data = net_large.fc3.bias.data[:small_rep_dim]
                        # ---- reuse the same header -----#
            ...
            results = [mean_small_loss, mean_small_acc, mean_large_trained_loss, mean_large_trained_acc] \
                      + [round(i,4) for i in PM_large_acc.values()] #+ [i for i in ALPHA.values()] #+ [i for i in LR_gate.values()]
    ...
    new_file = str(save_path / f"Done_Homo_FedMRL2_small_rep_dim_{small_rep_dim}_N{num_nodes}_C{fraction}_R{steps}_E{epochs}_B{bs}_NonIID_{data_name}.csv")
